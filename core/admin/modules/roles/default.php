<div class="table">
	...
	<header>
		<span class="view_column users_name"><a class="sort_column asc" href="ASC" name="role">Name <em>&#9650;</em></a></span>
		<span class="view_column users_name"></span>
		<span class="view_action" style="width: 80px;">Actions</span>
	</header>
	<ul id="results">
		<? include BigTree::path("admin/ajax/roles/get-page.php") ?>	
	</ul>
</div>

<script>
	BigTree.localSortColumn = "role";
	BigTree.localSortDirection = "ASC";
	BigTree.localSearchTimer = false;
	BigTree.localSearch = function() {
		$("#results").load("<?=ADMIN_ROOT?>ajax/roles/get-page/?sort=" + escape(BigTree.localSortColumn) + "&sort_direction=" + escape(BigTree.localSortDirection) + "&page=1&query=" + escape($("#query").val()));
	}
	...
	$(".table").on("click",".icon_delete",function() {
		...
		BigTreeDialog({
			title: "Delete Role",
			content: '<p class="confirm">Are you sure you want to delete this role?</p>',
			icon: "delete",
			alternateSaveText: "OK",
			callback: $.proxy(function() {
				$.ajax("<?=ADMIN_ROOT?>ajax/roles/delete/", { type: "POST", data: { id: $(this).attr("href").substr(1) } });
			},this)
		});
		...
		BigTree.localSortColumn = $(this).attr("role");
		...
		$("#results").load("<?=ADMIN_ROOT?>ajax/roles/get-page/?sort=" + escape(BigTree.localSortColumn) + "&sort_direction=" + escape(BigTree.localSortDirection) + "&page=" + BigTree.cleanHref($(this).attr("href")) + "&query=" + escape($("#query").val()));
